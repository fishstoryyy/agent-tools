...
4. **Stay off the "how."** Do not ask the user to choose implementation details such as architecture, libraries, data structures, or sequencing; use `/plan` or `grill-me-full` for that afterward. Goal-level questions about causality, adoption, measurement, ownership, source authority, and technical constraints are still in scope.
